...
/*
Notes:
while mouse is pressed, and reload = 100%, spawn new shell, aimed at mouse, it stores the mousePos vector and displays 
its own crosshair at some point work out rotation for the turret change boolean state to more complete, start menu with 
settings, in game(level), between level upgrade selection, game over screen with option to return to menu.
*/
...
void setup() {
    ...
    //so to display custom background for now
    //  level = 0;
    level = 1;  
